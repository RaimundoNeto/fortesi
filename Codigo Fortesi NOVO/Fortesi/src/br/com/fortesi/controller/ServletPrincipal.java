	...
	protected void service(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		if(request.getParameter("param").equals("autlogin")){
			...
			response.sendRedirect("http://localhost:8080/Fortesi/Jsp/Login.jsp");
				...
				else if(request.getParameter("param").equals("caddata")){
					String pegadata = request.getParameter("minhadata");
					
					try{
						String sqlinsere = "insert into testelista (datateste) values(?)";
						PreparedStatement pstm = conn.prepareStatement(sqlinsere);
						
						response.sendRedirect("http://localhost:8080/Fortesi/Jsp/testandodata.jsp");
